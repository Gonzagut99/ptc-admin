...
export type ServerMultiOptionInfinite = {
  label: string;
  value: string;
  icon?: React.ComponentType<{ className?: string }>;
};

interface ServerMultiFacetedFilterInfiniteProps {
  icon?: React.ComponentType<{ className?: string }>;
  title: string;
  options: ServerMultiOptionInfinite[];
  values?: string[];
...
export function ServerMultiFacetedFilterInfinite({
  title,
  options,
  values,
  onChange,
  ...
  searchPlaceholder,
  icon: Icon,
}: ServerMultiFacetedFilterInfiniteProps) {
      ...
      <PopoverContent className="w-[240px] p-0" align="start">
        <Command shouldFilter={false}>
          ...
          <CommandList ref={scrollRef} className="max-h-[300px]">
            ...
            {selectedSet.size > 0 && !isSearching && (
              <>
                <CommandSeparator />
                <CommandGroup>
                  <CommandItem
...
export default ServerMultiFacetedFilterInfinite;
