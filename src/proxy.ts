...
// Rutas que no deben ser procesadas por el middleware (evitar bucles)
...
const EXCLUDED_REDIRECT_ROUTES = ["/forbidden", "/auth/log-in", "/500", "/404"];
...
function isAuthRelatedRequest(request: NextRequest): boolean {
  ...
  if (pathname === "/auth/log-in" && method === "POST") return true;
...
export async function proxy(request: NextRequest) {
  ...
  // Evitar bucles de redirección - no procesar rutas excluidas
  if (EXCLUDED_ROUTES.includes(pathname)) {
    devlog(`Ruta excluida del middleware: ${pathname}`);
    return NextResponse.next();
  }
  ...
  if (isAuthRelatedRequest(request)) {
    devlog(
      "Petición relacionada con autenticación, permitiendo sin interferir",
    );
    return NextResponse.next();
  }

  // Verificar presencia de cookie de autenticación
  const sessionCookie = request.cookies.get(AUTH_COOKIE_NAME);

  // CASO 1: Ruta pública, usuario aparentemente autenticado -> redirigir a dashboard
  ...
  if (!PUBLIC_ROUTES.includes(pathname) && !sessionCookie) {
    ...
    const response = NextResponse.redirect(
      new URL("/auth/log-in", request.url),
    );
